...
export default class Fyd {
     ...
     * If no val is defined return array of attribute value
     * @param attr
     * @param val
     * @returns {*}
     */
     ...
     */
    remove(){
        this.target.forEach((el) => el.remove());
    }
     ...
     */
    first(){
        return this.target.filter( (i, k) => !k ? i : null);
    }
     ...
     */
    last(){
        return this.target.filter( (i, k) => k === this.target.length - 1 ? i : null);
    }
